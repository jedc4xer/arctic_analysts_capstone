...
def get_date_range(start_date, period_range, freq):
    ...
    date_list = pd.date_range(
        start = start_date,
        periods = period_range + 1,
        freq = freq
    ).to_pydatetime().tolist()
    
    date_list = [str(_.year) + "-" + str(_.month).zfill(2) + "-" + '15' for _ in date_list]
...
def create_poly_models(master_table, target, locale, age_group = "25-44"):
    ...
    for i in range(45, 1, -1):
        try:
            ...
            model, x_poly, poly_pred, rmse, future_predictions = create_and_run_models(X, y, i)
...
def income_data():
    ...
    master_df = master_df[['FIPS','Year','MedianHousePrice','MedianIncome','AgeGroup','Month']]
...
def create_and_run_models(X, y, degree):
    ...
    try:
        future_predictions = pd.DataFrame(zip(future_X['predictor'].tolist(), future_predictions), columns = ['predictor', 'futures'])
    except Exception as E:
        print(E)
    ...
    return pipe, x_poly, poly_pred, np.sqrt(mean_squared_error(y, poly_pred)), future_predictions
