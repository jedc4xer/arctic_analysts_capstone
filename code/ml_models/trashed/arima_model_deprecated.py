...
def graph_final_results(copy_for_graph, target):
    ...
    ax = sns.lineplot(
        x=copy_for_graph["Year"],
        y=copy_for_graph["converted"],
        linewidth=3,
        label="Predicted",
    )
    ax = sns.lineplot(
        x=copy_for_graph["Year"],
        y=copy_for_graph[target],
        linewidth=4,
        color="black",
        label="Ground Truth",
    )
...
def get_adf(df, target):
    ...
    print("\n###############################################\n")
    print(
        f"Best Result: Column: {col} | ADF Statistic: {adf_result[0]} | P-Value: {adf_result[1]} | Passed? {adf_result[1] <.05}"
    )
    print("\n###############################################\n")
    if best_col == "diff_12":
        num_diffs = 12
...
def undifference(df, num_diffs, target):
...
def control_arima(master_table, target, params):
    ...
    def ARIMA_predict(df, best_col, best_arima, num_periods, target, params):
        ...
        y_train, y_test = train_test_split(
            df[target].dropna(), train_size=0.75, shuffle=False
        )
    ...
    if target == "MedianIncome":
        ...
        best_arima = evaluate_models(
            adf_filtered_df[best_col].dropna(), [2, 3, 4, 5], [num_diffs], range(1, 3)
        )
